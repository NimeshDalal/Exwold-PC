...
namespace ITS.Exwold.Desktop
{
    class TCPServer
    {
        ...
        private void CheckConnection(object stateObject)
        {
            ...
            msg.Append("locIP=");
            msg.AppendLine(((TCPServer)stateObject).localIPAddress.ToString());
            msg.Append("locPort==");
            msg.AppendLine(((TCPServer)stateObject).localIPPort.ToString());
            ...
            {
                ((TCPServer)stateObject).log.LogMessage(ThreadLog.DebugLevel.Information, msg.ToString(), Logging.ThisMethod(), ex);
            }
            catch (Exception ex)
            {
                ((TCPServer)stateObject).log.LogMessage(ThreadLog.DebugLevel.Exception, msg.ToString(), Logging.ThisMethod(), ex);
            }
        }
